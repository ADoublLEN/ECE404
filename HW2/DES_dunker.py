...
if len(sys.argv) is not 4:
    sys.exit('''Usage: python DES_dunker.py message.txt key.txt [mode]\n\n\tModes: E - encrypt D - decrypt''')
elif (sys.argv[3] != 'E' != sys.argv[3] != 'D'):
    sys.exit('''Only modes are E - encrypt and D - decrypt''')
...
def get_encryption_key(): # key                                                              
    ...
    while len(key) != 8 or not key.isalpha():
        print("Key should be 8 characters.")
        if sys.version_info[0] == 3:
            key = input("\nEnter key: ")
        else:                                                                         
            key = raw_input("\nEnter key: ")
...
def main():
    ...
    des(sys.argv[3], sys.argv[1], sys.argv[2], key)
