...
def getKeySchedule(bv):
    ...
    #key = BitVector( hexstring = "2b7e151628aed2a6abf7158809cf4f3c" )
    #key = BitVector( hexstring = "000102030405060708090a0b0c0d0e0f")
    key = BitVector( textstring = inKey)
    words = [None, None, None, None]
    xbv = [None, None, None, None]
    [lefth, righth] = bv.divide_into_two()
    [xbv[0], xbv[1]] = lefth.divide_into_two()
    [xbv[2], xbv[3]] = righth.divide_into_two()
    ...
    for i in range(4,44,1):
        ...
        if i % 4 == 0:
            temp = RotWord(temp)
            temp = SubWord(temp)
            temp ^= roundConstant[(i/4)-1]
        kSchedule[i] = temp ^ kSchedule[i-4]
    for i in range(0,44,4):
        roundKeys[i/4] = kSchedule[i]+kSchedule[i+1]+kSchedule[i+2]+kSchedule[i+3]

def AES_enc(key, message, fileout):
    ## Get the key
    kBV = BitVector( textstring = inKey )
    #kBV = BitVector( hexstring = "2b7e151628aed2a6abf7158809cf4f3c")
    #kBV = BitVector( hexstring = "000102030405060708090a0b0c0d0e0f")
    ## Create the bitvector for the file
    bv = BitVector( filename = message )
    #bv = BitVector( hexstring = "3243f6a8885a308d313198a2e0370734")
    #bv = BitVector( hexstring = "00112233445566778899aabbccddeeff")
...
def AES_dec(key, message, fileout):
    ...
    bv = BitVector( filename = message )
    #kBV = BitVector( hexstring = "000102030405060708090a0b0c0d0e0f")
    #bv = BitVector( hexstring = "69c4e0d86a7b0430d8cdb78070b4c55a")
    FILEOUT = open(fileout, 'wb')
    stateArray = [[0 for x in range(4)] for x in range(4)]
    nextArray = [[0 for x in range(4)] for x in range(4)]
    while(bv.more_to_read):
...
def main():
    ...
    AES_enc(inKey, inmessage, encout)
    AES_dec(inKey, encout, decout)
